<?php
  ...
  if (!isset($_SESSION['details_change_success'])){
    header('Location: details_change.php');//Send user to the Details Change page
    exit();//Exit this file
  }else{//If the successReg flag exists in the session
    unset($_SESSION['details_change_success']);//Unset the 'details_change_success' variable so it doesn't
  }
...
EOL;
  ...
  <div id="sign-in-main">
    ...
    <div id="sign-in-form" class="container">
      <div class="heading">
        <h1>Thank you!</h1>
        <h3>Your details have been updated successfully</h3>
        <a href="myaccount.php"><h3>You can now change them on 'My Account' page</h3></a>
      </div>
    </div>
  </div>
